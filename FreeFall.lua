...
--global variables
_G.EspColor = Color3.new(255/255,0/255,0/255)
_G.Volume = 1
_G.speed = 5
--functions
...
local function falldmg()
    ...
    GMeta.__namecall = function(remote,arg1,arg2,arg3,...)
        if not _G.FallDamageEnabled == false then return __Namecall(remote,arg1,arg2,arg3,...) end
...
for i, v in pairs(workspace:GetChildren()) do
    if v:FindFirstChild("Demon") and v:FindFirstChild("Head") then
        ESP(v.PrimaryPart, v.Name, (_G.EspColor))
    end
end
...
workspace.DescendantAdded:Connect(function(desc)
    if desc.Parent:FindFirstChild("PickableItem") then
        ESP(desc.PrimaryPart, v.Name, (_G.EspColor))
    end
end)
end
...
local win = Flux:Window("Free Fall", "By StepBroFurious#3333", Color3.fromRGB(255, 110, 48), Enum.KeyCode.LeftAlt)
...
_G.noclip = true
falldmg()
To(Vector3.new(-1979.56, 871.601, -6505.75))
repeat
wait()
until (rootpart.Position - (Vector3.new(-1979.56, 871.601, -6505.75))).magnitude < 10
_G.noclip = false
end)
tp:Button("Slayer Exam", "", function(t)
_G.noclip = true
falldmg()
To(Vector3.new(-5193.77, 792.619, -3044.08))
repeat
wait()
until (rootpart.Position - (Vector3.new(-5193.77, 792.619, -3044.08))).magnitude < 10
_G.noclip = false
end)
...
local PlayerTable = {}
    for i, v in pairs(game.Players:GetChildren()) do
        ...
        _G.noclip = true
        falldmg()
        local targetpos = (game.Players[value].Character.HumanoidRootPart.Position + Vector3.new(0,3,0))
        To(targetpos)
        repeat
        wait()
        until (rootpart.Position - (targetpos)).magnitude < 10
        _G.noclip = false
        end)
        game.Players.PlayerAdded:Connect(function(player)
...
farm:Label("Mobfarm coming soon!!!")
...
credits:Label("Credit to racemodex for the trinket farm i didnt feel like making")
credits:Label("Credit to 3ds for the MT-API")
credits:Label("The only god I beleive in is 3ds")
credits:Button("V3rm post like it up", "", function(t)
