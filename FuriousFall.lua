...
--
local discord = "https://discord.gg/y5adzpUp"
...
--global variables
getgenv().EspColor = Color3.new(255/255,0/255,0/255)
getgenv().Volume = 1
getgenv().speed = 5
getgenv().WalkSpeedValue = 24
getgenv().JumpPowerValue = 60
getgenv().targetpos = 0
getgenv().folder = false
...
while wait(0.5) do
    for i, v in pairs(workspace:GetChildren()) do
        if v:FindFirstChild("PickableItem") and v:FindFirstChild("Main") and (v:FindFirstChild("Main").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 50 then
            game:GetService("ReplicatedStorage").Remotes.Async:FireServer("Character", "Interaction", v.Main)
        elseif v:FindFirstChild("PickableItem") and v:FindFirstChild("Part") and (v:FindFirstChild("Part").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 50 then
...
local mods = {"Helpers", "Moderators", "Admins", "Developers", "Torso", "Head"}

local function kickifmod()
    for i, v in pairs(game.Players:GetChildren()) do
    if v:IsInGroup(4294803) then
        local role = (v:GetRoleInGroup(4294803))
        if role == mods[1] then
        print(v,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        elseif role == mods[2] then
        print(v,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        elseif role == mods[3] then
        print(v,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        elseif role == mods[4] then
        print(v,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        elseif role == mods[5] then
        print(v,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        elseif role == mods[6] then
        print(v,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        end
    end
...
game.Players.PlayerAdded:Connect(function(player)
    if player:IsInGroup(4294803) then
        local role = (player:GetRoleInGroup(4294803))
        if role == mods[1] then
        print(player,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
        elseif role == mods[2] then
        print(player,"is a", role)
        game.Players.LocalPlayer:Kick("the feds are here nigga server hop")
...
getgenv().Tp = (Vector3.new(position.x, position.y, position.z))
spawn(function()
...
getgenv().YPos = hm.Position.y + 10
local dist = (hm.Position - position).magnitude
local tweenspeed = dist/tonumber(getgenv().speed*10)
local ti = TweenInfo.new(tonumber(tweenspeed), Enum.EasingStyle.Linear)
local tp = {CFrame = CFrame.new(position)}
getgenv().TweenThing = ts:Create(hm, ti, tp)
getgenv().TweenThing:Play()
wait(tonumber(tweenspeed))
getgenv().TweenThing = nil
end
end)
end
...
local function gourd()
    ...
    TextLabel:GetPropertyChangedSignal("Text"):Connect(function()
        local Text = tonumber(TextLabel.Text:match("%d+"))
        if Text < 50 then
             mouse1press()
        elseif Text > 70 then
            mouse1release()
...
local function falldmg()
    ...
    local __Namecall = GMeta.__namecall
    setreadonly(GMeta, false)
    getgenv().FallDamageEnabled = false

    GMeta.__namecall = function(remote,arg1,arg2,arg3,...)
        if not getgenv().FallDamageEnabled == false then return __Namecall(remote,arg1,arg2,arg3,...) end
    
        if tostring(remote) == 'Async' then
             if arg2 == "FallDamageServer" then
                 arg3 = 0
                 end
            end
        return __Namecall(remote,arg1,arg2,arg3,...)
    end
end

local function demonesp()
    function WTS(part)
...
for i, v in pairs(workspace:GetChildren()) do
    if v:FindFirstChild("Demon") and v:FindFirstChild("Head") then
        ESP(v.PrimaryPart, v.Name, (getgenv().EspColor))
    end
end
...
game.Players.LocalPlayer.Character.ChildAdded:Connect(function(child)
    if table.find(badtable, child.Name) then
        wait(0.01)
        child:Destroy()
    elseif string.find(child.Name, "Cooldown") then
        wait(0.01)
        child:Destroy()
    end
end)
...
for i, v in pairs(workspace.Npcs:GetChildren()) do
      if v:FindFirstChild("NPC") and v:FindFirstChild("Head") and not v:FindFirstChild("Head"):FindFirstChild("aaaaaa") then
        ESP(v.PrimaryPart, v.Name, (getgenv().EspColor))
    end
end
...
workspace.DescendantAdded:Connect(function(desc)
    if desc.Parent:FindFirstChild("PickableItem") then
        ESP(desc.PrimaryPart, v.Name, (getgenv().EspColor))
    end
end)

if _G.folder then
    for i, v in pairs(game.Workspace.AutoTrinket:GetChildren()) do
    if v:FindFirstChild("Main") and v:FindFirstChild("PickableItem") and not v:FindFirstChild("Main"):FindFirstChild("aaaaaa") then
        ESP(v.PrimaryPart, v.Name, (getgenv().EspColor))
        elseif v:FindFirstChild("Part") and v:FindFirstChild("PickableItem") and not v:FindFirstChild("Part"):FindFirstChild("aaaaaa") then
        ESP(v.PrimaryPart, v.Name, (getgenv().EspColor))
...
local function playerespbutton()
    ...
    function playerEsp()
        ...
        RenderStepped = game:GetService("RunService").RenderStepped:Connect(function()
            ...
            if player.Character ~= nil and player.Character:FindFirstChild("HumanoidRootPart") ~= nil then
                esp.Color = (getgenv().EspColor)
                esp.Text = tostring(player.Name) .. "\n" .. tostring(math.ceil(player.Character.Health.Value))
...
local closest = nil--Part, distance
-------------Loop thru
for i,v in pairs(folder:GetChildren()) do
    local distance = (rootpart.Position - v.Part.Position).magnitude
...
getgenv().noclip = true
getgenv().targetpos = closest.Position + Vector3.new(0,3,0)
To(getgenv().targetpos)
repeat
wait()
until (rootpart.Position - (getgenv().targetpos)).magnitude < 6
closest.Parent.Parent = workspace
for i, v in pairs(workspace:GetChildren()) do
    if v:FindFirstChild("PickableItem") and v:FindFirstChild("Main") and (v:FindFirstChild("Main").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 50 then
        game:GetService("ReplicatedStorage").Remotes.Async:FireServer("Character", "Interaction", v.Main)
    elseif v:FindFirstChild("PickableItem") and v:FindFirstChild("Part") and (v:FindFirstChild("Part").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 50 then
...
--tabs
local rootpart = game.Players.LocalPlayer.Character.HumanoidRootPart
local win = Flux:Window("Furious Fall", "By StepBroFurious#3333", Color3.fromRGB(255, 110, 48), Enum.KeyCode.LeftAlt)
local esp = win:Tab("Visuals", "http://www.roblox.com/asset/?id=6022668888")
local tp = win:Tab("Teleports", "http://www.roblox.com/asset/?id=6022668888")
local farm = win:Tab("Auto Farm", "http://www.roblox.com/asset/?id=6022668888")
local misc = win:Tab("Misc", "http://www.roblox.com/asset/?id=6022668888")
local music = win:Tab("Music", "http://www.roblox.com/asset/?id=6022668888")
local credits = win:Tab("Credits", "http://www.roblox.com/asset/?id=6035202033")

--esptab
esp:Button("Trinket Esp", "", function(t)
trinketesp()
end)
esp:Colorpicker("ESP Color", Color3.fromRGB(255,1,1), function(t)
getgenv().EspColor = (t)
end)
esp:Button("Player Esp", "", function(t)
playerespbutton()
end)
esp:Button("Demon Esp", "", function(t)
demonesp()
end)
esp:Button("NPC Esp", "", function(t)
npcesp()
end)
--tptab
tp:Slider("Tp Speed", "", 1, 15,5,function(t)
getgenv().speed = (t)
end)
tp:Label("Places")

tp:Button("Hayakama Village", "", function(t)
getgenv().noclip = true
falldmg()
To(Vector3.new(464.684, 756.253, -2016.01))
repeat
wait()
until (rootpart.Position - (Vector3.new(464.684, 756.253, -2016.01))).magnitude < 10
getgenv().noclip = false
end)

tp:Button("Okuiya Village", "", function(t)
getgenv().noclip = true
falldmg()
To(Vector3.new(-3163.0927734375, 703.95404052734, -1154.0573730469))
repeat
wait()
until (rootpart.Position - (Vector3.new(-3163.0927734375, 703.95404052734, -1154.0573730469))).magnitude < 10
getgenv().noclip = false
end)
tp:Button("Kamakura Village", "", function(t)
getgenv().noclip = true
falldmg()
To(Vector3.new(-2369.6340332031, 1161.8195800781, -1491.302734375))
repeat
wait()
until (rootpart.Position - (Vector3.new(-2369.6340332031, 1161.8195800781, -1491.302734375))).magnitude < 10
getgenv().noclip = false
end)
tp:Button("Slayer Corp", "", function(t)
getgenv().noclip = true
falldmg()
getgenv().targetpos = Vector3.new(-1979.56, 871.601, -6505.75) + Vector3.new(0,3,0)
To(getgenv().targetpos)
repeat
wait()
until (rootpart.Position - (getgenv().targetpos)).magnitude < 10
getgenv().noclip = false
end)
tp:Button("Slayer Exam", "", function(t)
getgenv().noclip = true
falldmg()
getgenv().targetpos = Vector3.new(-5193.77, 792.619, -3044.08) + Vector3.new(0,3,0)
To(getgenv().targetpos)
repeat
wait()
until (rootpart.Position - (getgenv().targetpos)).magnitude < 10
...
tp:Label("Player Teleports")
local PlayerTable = {}
    for i, v in pairs(game.Players:GetChildren()) do
        table.insert(PlayerTable, v.Name)
    end
        local ToPlayer = tp:Dropdown("To Player", PlayerTable, function(value)
        getgenv().noclip = true
        falldmg()
        getgenv().targetpos = (game.Players[value].Character.HumanoidRootPart.Position + Vector3.new(0,3,0))
        To(getgenv().targetpos)
        repeat
        wait()
        until (rootpart.Position - (getgenv().targetpos)).magnitude < 10
...
tp:Label("Npc Teleports coming soon!!!")


--autofarmstab
farm:Button("Trinket Farm", "", function(t)
trinketfarm()
end)

farm:Button("Auto Gourd", "", function(t)
gourd()
end)
...
misc:Slider("WalkSpeed", "", 16, 120,0,function(t)
getgenv().WalkSpeedValue = (t)
end)

misc:Slider("JumpPower", "", 60, 120,0,function(t)
getgenv().JumpPowerValue = (t)
end)
misc:Button("Combat", "Autobreathing, Godmode, No cooldown", function(t)
breath()
end)

misc:Button("ClearSight", "Clear fog", function(t)
sight()
end)

misc:Button("Anti Fall Damage", "No fall damage", function(t)
falldmg()
end)

misc:Button("Auto Pickup", "Picks up trinkets close to you automatically", function(t)
autopickup()
end)

misc:Button("Kick if mod joins", "", function(t)
kickifmod()
end)

misc:Button("Join Different Server", "", function(t)
Teleport()
end)
music:Slider("Volume", "", 0, 5,1,function(t)
getgenv().Volume = (t)
local soundplayer = workspace:FindFirstChild("MusicPlayer")
soundplayer.Volume = getgenv().Volume
end)

music:Textbox("Song Id", "", true, function(t)
local Sound = Instance.new("Sound")
Sound.Name = "MusicPlayer"
Sound.Parent = game.Workspace

Sound.SoundId = ("rbxassetid://"..t)
Sound:Play()
...
music:Button("Stop Song", "", function(t)
local soundplayer = workspace:FindFirstChild("MusicPlayer")
soundplayer:Destroy()
end)


credits:Label("The only god I beleive in is 3ds")
credits:Button("Copy Discord", "", function(t)
setclipboard(discord)
end)
credits:Button("Copy V3rm Post", "", function(t)
setclipboard("https://v3rmillion.net/showthread.php?tid=1144572")
end)